...
import { Call, lock, LOGO_IMAGE } from '../../Media/Media';
...
const SignIn = () => {
  ...
  const setFullName = AuthUsers((state) => state.setFullName);
  const setMobileNumber = AuthUsers((state) => state.setMobileNumber);
  ...
  const onSubmit= async (data) => {
    try {
      setLoading(true);
      setShowLoader(true); // Show loader on submit
      ...
      // Always hide loader and loading, regardless of success or error
      setShowLoader(false);
      setLoading(false);
    }
  };
    ...
    <div className="relative flex justify-center items-center overflow-x-hidden">
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          />
          <h4 className="text-xl sm:text-2xl font-bold text-center text-[#000000]">Log In</h4>
        </div>

        {/* Phone Number Input */}
        <div className="mt-8">
          <label className="block mb-1 text-sm text-[#898989]">Phone Number</label>
            ...
            <input
              ...
              {...register("mobile", {
                ...
                validate: (value) => {
                  ...
                  return onlyNumbers.test(value) || "Mobile Number must be exactly 10 digits";
                },
              })}
              onInput={(e) => {
        ...
        <div className="mt-6">
          ...
          <div className="text-right mt-2">
            <Link to="/auth/forget" className="text-sm text-[#012653] hover:underline hover:text-[#000000]">
              Forget Password?
            </Link>
          </div>
        </div>
        ...
        <div className="flex items-center justify-center mt-16 gap-1">
          ...
          <Link className="hover:text-[#8f25f8b4]" to={"/auth/signup"} style={{color:`var(--common_text_color)`}}>
            Sign up
          </Link>
        </div>
      </form>
