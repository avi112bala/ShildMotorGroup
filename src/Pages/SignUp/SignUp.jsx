...
import { Call, lock, LOGO_IMAGE, User } from "../../Media/Media";
...
const SignUp = () => {
  ...
  const setPassword = AuthUsers((state) => state.setPassword);
  ...
  const onSubmit = async (data) => {
    ...
    try {
      setLoading(true);
      const reqData = { type: "UR", mobile: data.mobile };
      // const res = await generateOtp(reqData);
      if (res.status === 200) {
    ...
    <div className="flex justify-center items-center">
      <div className="w-full min-h-screen max-w-xl p-8 bg-white shadow-lg relative  overflow-scroll h-[88vh]">
        <div className="text-center mb-4">
          ...
          />
          <h4 className="text-xl sm:text-2xl font-bold text-center text-[#000000]">Sign Up</h4>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="flex items-center justify-center mt-14 mb-6 gap-1">
            ...
            <Link className=" hover:text-[#8f25f8b4]" to={"/auth/signin"} style={{color:`var(--common_text_color)`}}>
              Log In
            </Link>
          </div>
        </form>
